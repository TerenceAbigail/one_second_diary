...
void main() {
  group('validate', () {
    ...
    test(
      'rejects a whitespace-only name — trimming it must not silently produce the '
    ...
    test(
      'rejects a trailing tab as an invalid character rather than trimming it away',
      () {
    ...
    test('rejects the localized default label too', () {
      expect(
        ProfileNameValidator.validate(
          'Estandar',
          existingLabels: const [],
          localizedDefaultLabel: 'Estandar',
        ),
        ProfileNameError.reserved,
      );
    });
    ...
    test(
      'rejects a name that collides with an existing profile, case-insensitively',
      () {
        expect(
          ProfileNameValidator.validate(
            'bob',
            existingLabels: const ['Bob'],
            localizedDefaultLabel: 'Default',
          ),
    ...
    test(
      'ignores leading/trailing whitespace when checking for duplicates',
      () {
        expect(
          ProfileNameValidator.validate(
            '  Bob  ',
